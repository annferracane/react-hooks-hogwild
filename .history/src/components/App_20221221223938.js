...
function App() {
	...
	const [hiddenHogs, setDisplayHogs] = useState(hogs);

	function filterHogs(filterVal) {
		
		const newHogs = [...displayHogs];
	...
	function hideHog(hogName) {
        const newHogList = displayHogs.filter(hog => hog.name !== hogName);
